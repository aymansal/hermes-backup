...
For Ayman, this is mandatory commander behavior, not optional polish: announce when a worker card starts, when a worker completes or blocks, when the reviewer starts, and when review returns PASS/BLOCKED. Do not wait for Ayman to ask "what happened?" after a card finishes; a quiet phase transition is a workflow failure. If a worker blocks with `review-required`, immediately inspect the handoff, complete it explicitly as `review-required, not PASS`, dispatch/start the dependent reviewer, and report the reviewer card id/status. For high-stakes raids, also announce activation/config writes, cron creation, restart/deploy approval gates, crash/reclaim/retry events, and decision gates as soon as they happen. If Ayman says he is waiting, perform a short active check and report the state; do not disappear into long polling loops that make the chat unavailable. If phase-change notifications are not arriving and manual babysitting becomes necessary, check notifier health and repair the board instead of normalizing polling. Also remember that current gateway notifications may cover only terminal/attention events, not `claimed`/`spawned` worker-start events; after CLI card creation, explicitly `notify-subscribe` worker and reviewer tasks to the current Telegram topic if Ayman expects pings. See `references/ayman-kanban-availability-and-notifier-health.md` and `references/kanban-notifier-subscriptions-and-phase-events.md`.

**Do not turn Kanban into invisible inline work.** Ayman's Kanban rule also means the main chat stays available. After dispatching a worker/reviewer, do one short state verification, report the card id/status/current battle line, and return control to chat. Do not sit in long polling loops unless Ayman explicitly asks for live monitoring or says he is waiting. If Ayman asks `Update?`, actively poll the relevant card(s) once, handle any already-completed PASS/BLOCKED transition, then report. If Ayman says he is waiting, actively poll until the worker/reviewer completes or blocks, then report immediately.

**No silent polling while Ayman is actively waiting.** After Ayman says "continue", "update", or otherwise watches the raid live, do not disappear for a long polling loop just because a worker/reviewer is still `running`. If there is no verdict after roughly 2–3 minutes, send a concise checkpoint: card id, current status, last heartbeat/log line, elapsed time, and whether you are still waiting or suspect stall. A six-minute silence during Kanban monitoring reads as operator drift even when the board is merely running. Long waits should be backgrounded or reported as "still running; next checkpoint at X" rather than hidden inside one tool call.
...
**Kanban SQLite DB corruption / notification failure.** If the gateway log spams "kanban notifier tick failed: database disk image is malformed" every ~5 seconds, or `hermes kanban --board <board> notify-list` throws `sqlite3.DatabaseError: database disk image is malformed`, the kanban SQLite database/notifier path is corrupted. This may not block card execution, but it disables or degrades phase-change notifications and tempts the General into manual polling. For Ayman, that is a workflow failure because Kanban exists to keep chat available. Recovery: see `references/kanban-db-corruption-recovery.md` and `references/ayman-kanban-availability-and-notifier-health.md`.

**Kanban notifier `.lower()` crash on malformed notify subscriptions.** If the gateway is alive but logs spam `kanban notifier tick failed: 'int' object has no attribute 'lower'`, inspect `kanban_notify_subs` across boards for non-string `platform` values before deleting anything. A robust code fix is to normalize adapter and subscription platform values via `str(...).lower()` inside `gateway/run.py::_kanban_notifier_watcher`, add a regression test with a numeric platform row, then restart the gateway only after Ayman approves. Details: `references/kanban-notifier-malformed-subscriptions.md`.
