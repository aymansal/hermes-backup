...
For non-Tailscale access to a local/static preview, use `references/temporary-public-preview-tunnels.md` and `references/public-preview-tunnels.md`: verify local health first, try public IP only if appropriate, ask approval before exposing a temporary public tunnel, prefer `cloudflared tunnel --url http://127.0.0.1:<PORT>`, parse `--logfile` if background stdout is empty, verify HTTP 200 on the generated URL and the specific app route/CSS the user will show, and remind Ayman that anyone with the link can view it while the tunnel runs. For pnpm workspace Next apps, pass `dev --hostname ... --port ...` directly; avoid an extra `--` after `dev` because Next may treat `--hostname` as the project directory.

When running commands from Telegram/Hermes that can finish within the current turn, prefer foreground `terminal()` or `background=true` followed by `process.wait` without `notify_on_complete`. Ayman dislikes surprise raw background-process completion dumps in the conversation. Use `notify_on_complete=true` only for genuinely long work where an autonomous completion alert is useful, and summarize results cleanly yourself. If noisy completion messages appear, explain that they come from Hermes background process notifications (`display.background_process_notifications`) and recommend `error` mode rather than changing config without approval.
