...
17. **Short-command handling** — `Update?` means give status/blockers/action/next gate from board/repo truth; `Go` means select and dispatch the next sensible card/gate without asking if the next move is obvious. If a worker is `blocked` only for `review-required`, add a routing comment and complete it as handoff to unlock the dependent review card; state that handoff completion is not approval. For completion percentages, report raw Kanban/card percentage separately from weighted MVP/product readiness. See `references/immopilot-progress-status-review-routing.md`.
18. **Revival Stone checkpoint** — for ImmoPilot repo raids, commit only after a reviewed `PASS`, then push the reviewed commit to GitHub before starting the next repo-changing raid. Include review/gate evidence in the commit body, verify the pushed branch, and start the next slice from a clean working tree. Do not carry uncommitted reviewed changes into the next slice. If GitHub remote is missing, stop and create/configure the private remote with Ayman's approval before declaring the checkpoint sealed. See `references/immopilot-kanban-github-finalization.md`.
...
- For unwired module shells such as Documents or Bons de livraison, static category/status definitions and empty layouts are acceptable, but fake business rows are not. Render an honest empty/unwired state until real tenant-scoped data is connected; fake document/BL records, fake filenames/numbers/dates/entities/metrics, and Sobhi hardcoding are review blockers. Enabled no-op controls are also blockers: search/filter/buttons must be genuinely functional over the displayed data or visibly disabled/non-misleading. See `references/immopilot-documents-shell-fake-data-review.md` and `references/immopilot-progress-status-review-routing.md`.
- For dashboard/report aggregates, treat every amount-bearing field as financial even when it is nested under status summaries or apartment/inventory metrics. Operational users may receive counts/status/surface summaries only. Money totals require `reports.viewFinancial` plus a validated bounded date range, and must be computed from date-filtered records only. Do not return all-time `apartments.totalPrice` or `financial.byStatus` amount totals. See `references/immopilot-dashboard-aggregate-finance-safety.md`.
- For dashboard UI wired to real metrics, validate financial date ranges locally before sending `dateFrom`/`dateTo`, never call Convex with placeholder organization IDs such as `dev-org`/`stub-org`, and render honest unavailable/permission/date-range states instead of fake zeroes or backend BAD_REQUEST crashes. See `references/immopilot-dashboard-ui-real-metrics-safety.md`.
...
- `references/immopilot-progress-status-review-routing.md` — progress/status doctrine for Ayman's short `Update?`/`Go` commands, review-required handoff routing, weighted MVP completion percentages, and no-op UI blocker lessons.
- `references/immopilot-dashboard-aggregate-finance-safety.md` — dashboard aggregate safety lesson: amount-bearing metrics need financial permission plus bounded date range; nested `byStatus` totals and apartment price totals can leak all-time money if not gated/date-filtered.
- `references/immopilot-dashboard-ui-real-metrics-safety.md` — dashboard UI safety lesson: real-data dashboards must validate date ranges before Convex calls, block placeholder org IDs, and show honest unavailable/permission states instead of fake zeroes or backend errors.
- `references/immopilot-documents-shell-fake-data-review.md` — Documents route shell lesson: static category definitions and honest empty/unwired states are allowed, but fake document rows/filenames/dates/metrics and Sobhi hardcoding are review blockers.
- `references/immopilot-carry-on-kanban-resume.md` — resume pattern for short “carry on with ImmoPilot” commands: inspect repo/board truth, avoid stale blocked-card resurrection, continue from reviewed doctrine/audit sequence, and create worker+review cards quickly.
