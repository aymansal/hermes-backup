...
Use this skill when you encounter issues with slash commands in the Hermes TUI, particularly when commands aren't showing in autocomplete, aren't working properly in the TUI, or need to be added/updated. Also use it for the native dashboard's embedded Chat tab because that path reuses the TUI gateway/PTY bridge and shares many of the same failure modes.
...
- The Shadow Realm / native dashboard Chat tab shows `[session ended]`, cannot start/continue sessions, or says `events feed disconnected — tool calls may not appear`
...
### Native dashboard embedded chat path

The Shadow Realm / native dashboard Chat tab has one more layer around the same TUI stack:

```
web/src/pages/ChatPage.tsx
       │  browser WebSocket
       ▼
hermes_cli/web_server.py:/api/pty        <- PTY-over-WebSocket bridge
       │  spawns
       ▼
hermes --tui / ui-tui                    <- Ink terminal UI
       │
       ▼
tui_gateway/server.py                    <- JSON-RPC, slash commands, events
```

The sidebar/tool-call feed is separate from the terminal PTY stream:

```
PTY child tui_gateway.entry
       │ HERMES_TUI_SIDECAR_URL
       ▼
/api/pub  →  /api/events  →  web/src/components/ChatSidebar.tsx
```

If the dashboard page loads but chat says `[session ended]` and the sidebar says `events feed disconnected — tool calls may not appear`, inspect the WebSocket gates (`/api/pty`, `/api/events`, `/api/pub`) before chasing model/provider issues. See `references/dashboard-embedded-chat-websocket-403.md` for a concrete Tailscale/non-loopback 403 diagnostic and fix pattern.

## Investigation Steps

1. **If the native dashboard Chat tab ends immediately, split HTTP health from WebSocket health first:**
   ```bash
   ...
   ```
   A `200 OK` page load only proves the SPA is served. It does not prove `/api/pty`, `/api/events`, or `/api/pub` WebSockets are accepted. For WebSocket 403 on Tailscale/private-IP binds, use the probe and fix notes in `references/dashboard-embedded-chat-websocket-403.md`.

2. **Check if the command exists in the TUI frontend:**
   ```bash
   search_files --pattern "/commandname" --file_glob "*.ts" --path ui-tui/
   search_files --pattern "/commandname" --file_glob "*.tsx" --path ui-tui/
   ```

3. **Examine the TUI command definition:**
   ```bash
...
4. **Check if the command exists in the Python backend:**
   ```bash
...
5. **Examine the gateway implementation:**
   ```bash
